...
for k = 1:N
    ...
    Ypred(k) = [height(k) leg(k)]*thetahat;
    thetav(:,k) = thetahat;
end

% Uncomment this to study results

rmseLOOCV  = sqrt(mean((Ypred-shoe).^2))      % Leave-one-out CV performance
rmse1 = sqrt(mean((Ypred1-shoe).^2))     
rmse2 = sqrt(mean((Ypred2-shoe).^2))
