...
rewards = struct('default', -1, 'apple', 5, 'death', -20); % Reward signal
gamm    = 0.95;  % Discount factor in Q-learning
alph    = 0.9; % Learning rate in Q-learning (automatically set to zero during testing)
eps     = 0.5; % Random action selection probability in epsilon-greedy Q-learning (automatically set to zero during testing)

% Optionally play around also with these settings.
alph_update_iter   = 400;   % 0: Never update alpha, Positive integer k: Update alpha every kth episode
alph_update_factor = 0.8; % At alpha update: new alpha = old alpha * alph_update_factor
...
for i = 1 : nbr_ep
    ...
    while 1
        ...
        if terminate
            ...
            % this is a terminal state, so we do not count future rewards.
            % The prediction for future reward is the Q_val of this
            % position and action
        ...
        % this state is not terminal so here we give reward + future
        % discounted optimal rewards. 
