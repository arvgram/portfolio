...
for action = 1 : 3 % Evaluate all the different actions (left, forward, right).
    ...
    state_action_feats(1, action) = grid(next_head_loc(1), next_head_loc(2))...
        /max(max(grid));
    
    % feature 2 and 3: horizontal and vertical distance to apple
    [r,c] = find(grid == -1);
    state_action_feats(2, action) = abs((next_head_loc(1)-r)/N); 
    state_action_feats(3, action) = abs((next_head_loc(2)-c)/N);
 %   switch action
 %       case 1
 %           state_action_feats(4, action) = -0.1;
 %       case 2
 %           state_action_feats(4, action) = 0.1;
 %       case 3
 %           state_action_feats(4, action) = -0.1;
