...
for kframe = 1:Nframes % First loop over frames
    ...
    for kfold = 1:K % Then loop over the folds
        ...
        valind = cvlocation+1:cvlocation+cvhop; % Select validation indices
        estind = [1:cvlocation cvlocation+cvhop+1:N]; % Select estimation indices
        ...
        for klam = 1:Nlam  % Finally loop over the lambda grid
            ...
            SEval(kfold,klam) = SEval(kfold,klam) + norm(t_val - X_val*what,2)^2/length(t_val); % Add validation error at current frame, fold and lambda to the validation error for this fold and lambda, summing tthe error over the frames
            SEest(kfold,klam) = SEest(kfold,klam) + norm(t_est - X_est*what,2)^2/length(t_est); % Add estimation error at current frame, fold and lambda to the estimation error for this fold and lambda, summing tthe error over the frames
            
            wold = what; % Set current LASSO estimate as estimate for warm-starting.
            disp(['Frame: '    num2str(kframe) '/' num2str(Nframes) ',' ...
                ' Fold: '      num2str(kfold) '/' num2str(K) ', ' ...
                'Hyperparam: ' num2str(klam) '/' num2str(Nlam)]) % Display progress through frames, folds and lambda-indices.
        end
...
[~,mindex] = min(MSEval);
lambdaopt = lambdavec(mindex); % Assign optimal lambda 
