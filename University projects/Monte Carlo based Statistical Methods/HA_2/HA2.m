...
%% 3. First approach, sample large N random walks and see which ones are SAW
N = 10000;
d = 2;
n = 20;
nbr_saw = 0;
...
var = est*((2*d)^n-est)/N;
CI = [est-1.96*sqrt(var) est+1.96*sqrt(var)];
...
% idea: take one step at the time with every particle before next step
%   for every time step
%       for all particles
%           get position
...
%self_avoiding = length(X) == length(unique(X,"rows"));
%disp("Probability of getting stuck on walk of length " + n + " is: " + ...
%    mean(has_been_stuck) + " (N = " + N + ")")
...
for k = 1:n
    for i = 1:N
        ...
        nextposition = FN(randi(nbr_available,1),:);
        end

        weights(k+1,i) = weights(k,i)*nbr_available;
        X(k+1,:,i) = nextposition;
    end
...
Xreg = [ones(n,1) m' log(m)'];
Yreg = log(cn(1:end));

[B, BINT] = regress(Yreg,Xreg);
